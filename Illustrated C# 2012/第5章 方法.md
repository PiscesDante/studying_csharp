...
`ReturnType MethodName(Parameter List) { Method Body }`
...
* 隐式初始化：初始化成该类型的默认值；
* 存储区域：由于实例字段是类的成员，所以所有的字段都应该在**<u>堆内存</u>**里，无论它们是值类型还是引用类型；
...
`var`关键字不像JavaScript里的`var`那样可以引用不同的类型。它是从等号右边推断出的实际类型的速记，有点类似C++中的`auto`关键字。

### 5.3.2 嵌套块中的本地变量

重要说明：**<u>在C和C++中，可以先声明一个本地变量，然后在嵌套块中声明另一个相同名称的本地变量。在内部范围，内部名称隐藏了外部名称。然而，在C#中不管嵌套级别如何，都不能在第一个名称的有效范围内声明另一个同名的本地变量</u>**。
...
* 在类型之前增加关键字`const`。
* 必须有初始化语句。初始化值必须在编译期决定，通常是一个预定义简单类型或由其组成的表达式。
...
public class MyClass
{
    ...
    {
        DateTime dt = DateTime.Now;
        if (dt.Hour < 12) return; // 如果没有超过中午，返回
...
{
    int sum = x + y;
    Console.WriteLine("Newsflash: {0} = {1} + {2}", sum, x, y);
}
...
namespace myApp
{
    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            float fv2 = 5.235f;
            int val1 = 15;
            Console.WriteLine("Newsflash: Sum: {0} and {1} is {2}", val1, 90, Calculator.Sum(val1, 90));
            Console.WriteLine("Newsflash: Avg: {0} and {1} is {2}", fv1, fv2, Calculator.Avg(fv1, fv2));
        }
    }
}
...
public class Program
{
    ...
    public static void Main()
    {
        MyClass mc = new MyClass();
        int val = 10;
        Console.WriteLine("mc.val = {0}, val = {1}", mc.val, val);
        func(mc, val);
        Console.WriteLine("mc.val = {0}, val = {1}", mc.val, val);
    }
}
...
这段程序表明：**<u>如果传入的是引用类型，那么发生复制的也仅仅是引用本身（也就是指针本身，而不是其所指向的数据），于是形参也会复制一份这个指针，也就是说此时形参和实参指向了同一块堆内存区域。当使用形参在函数内进行对数据的修改时，也会影响实参指向的数据（因为指向了同一块内存）</u>**。

## 5.11 引用参数

使用引用参数时，必须在方法的声明和调用中都使用`ref`修饰符。

实参必须是变量，在用作实参之前必须被赋值。如果是引用类型变量，可以赋值为一个引用或者`null`。

```C#
void MyMethod(ref int val) { ... } // 方法声明：必须包含ref修饰符
...
public class Program
{
    public static void Room(ref Person ps) // 引用类型的引用
    {
...
方法重载的核心在于每个相同名称的方法**<u>必须有一个独一无二的签名</u>**。

方法的签名由下列信息组成，它们在方法头中：
...
注意：**<u>返回类型</u>**和**<u>形参名称不是</u>**签名的一部分。
...
public class Program
{
    public static int CalcSum(int x = 5, int y) // 现在是非法的
    {
        // 可选参数必须出现在所有必需参数之后
        // 也就是说，一个有默认值的形参的右边的形参都必须有默认值
        return x + y;
    }
...
除了值类型和引用类型（默认值只能是`null`），其余参数类型均不可设置默认值。
...
**在方法调用时，整个栈帧都会压入栈。方法退出的时候，整个栈帧都会从栈上弹出来。弹出栈帧有的时候也叫做栈展开**。
